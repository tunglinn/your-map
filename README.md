...
- Youbike stations (live, Taipei) — official city feed, fetched directly (CORS-enabled), refreshed on load. Marker is a small square 🚲 badge whose color is the availability signal (green/amber/red); hidden entirely below zoom 16 to stay calm when zoomed out. Square badges vs. the round search-result pin is deliberate — square marks a category/ambient layer, round marks something you picked/searched.
- Metro/rail stations (🚇) — always shown regardless of zoom (small dataset), bigger badge than the other layers since it's meant to read as a landmark. Badge color is that station's real MRT/LRT line color (from OSM route relations); interchange stations (2+ lines) show as vertical color stripes instead of one solid color. Stations with no matched line (TRA/THSR, and some New Taipei Metro stations not yet captured — see `data/README.md`) fall back to a neutral color.
